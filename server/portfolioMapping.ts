...
export function computeRiskScore(inputs: RiskProfileInputs): RiskScore {
  const overall = 
    0.40 * inputs.riskTolerance +
    0.30 * inputs.riskCapacity +
    0.20 * inputs.investmentHorizon +
    0.10 * inputs.investorExperience;
...
export function computeTargetRegionMix(
  ...
  const allRegions: GeographicRegion[] = ['NL', 'EU_EX_NL', 'US', 'DEV_EX_US_EU', 'EM'];
  ...
  if (selectedRegions.length === 0) {
    return {
      ...
      US: 0.60,        // ~60% US (largest market)
      DEV_EX_US_EU: 0.15, // ~15% Developed ex-US/EU (Japan, UK, etc.)
      EM: 0.10,        // ~10% Emerging markets
    };
  }
 ...
 */
export function mapRiskProfileToParams(inputs: RiskProfileInputs): OptimizationParams {
  ...
  return {
    ...
    maxWeight: 0.40, // Max 40% in single ETF
  };
}
...
export function assessmentToRiskProfile(answers: any): RiskProfileInputs {
  ...
  const industryExclusions = (answers.esgExclusions || [])
    .filter((e: string) => e !== 'NO_ESG_SCREEN');
